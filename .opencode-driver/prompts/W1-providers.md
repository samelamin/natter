...
- Reuse the existing Redis cache if trivial: import `{ cacheGet, cacheSet }` from `lib/cache.js` IF those exports exist (check first — read lib/cache.js). Cache search results keyed `prov:{domain}:{sha-ish of params}` TTL 6h. If cache.js API differs, skip caching rather than guess — correctness over cleverness.
