...
export async function domainRecommend({
  ...
  try {
      ...
      for (const r of results) {
        if (r.status === 'fulfilled') {
          ...
          if (inner.status === 'fulfilled' && Array.isArray(inner.value)) {
            ...
            // Games: missing IGDB creds OR any IGDB error (service down, auth
            // failure) → fall back to LLM-sourced real game titles.
    ...
    // Games came back thin (IGDB unavailable, or the plan's title guesses
    // missed): ask the LLM for real game titles that fit the vibe, then — when
    // IGDB is available — look each up to attach real cover art + scores.
    if (domain === 'game' && candidates.length < 3) {
      const synth = await generateGameCandidates(query, llmClient);
      let enriched = [];
      if (provider && typeof provider.search === 'function' && synth.length) {
        const lookups = await Promise.allSettled(
          synth.slice(0, 12).map((s) => provider.search({ query: s.title, limit: 1 })),
        );
        enriched = lookups.flatMap((r) =>
          r.status === 'fulfilled' && Array.isArray(r.value) && r.value[0] ? [r.value[0]] : [],
        );
      }
      if (enriched.length) {
        candidates = candidates.concat(enriched);
      } else {
        // No IGDB data at all — use the LLM titles (no cover art / scores).
