...
const PLAN_HINTS = {
  ...
  game: 'searchTerms = game titles or genre keywords. filters: optional {"genres":"action,indie", "ordering":"-rating", "dates":"YYYY-MM-DD,YYYY-MM-DD"}.',
...
/**
 * Game fallback when RAWG_API_KEY is absent: ask the LLM for real, existing
 * game titles matching the query. Returns unified picks (no cover art /
 * metacritic — those need RAWG). Far better UX than web-article links.
 */
...
export async function domainRecommend({
  ...
  try {
      ...
      for (const r of results) {
        if (r.status === 'fulfilled') {
          ...
          if (inner.status === 'fulfilled' && Array.isArray(inner.value)) {
            ...
            // Games: missing RAWG key OR any RAWG error (service down, rate
            // limit) → fall back to LLM-sourced real game titles.
            if (domain === 'game') usedBraveFallback = true;
          }
        }
      }
    }

    // Games with no RAWG key: synthesize real game titles from the LLM's own
    // knowledge (no cover art / metacritic until RAWG_API_KEY is set).
    ...
    // Note the degraded mode when games ran without a RAWG key.
    const finalIntent = usedBraveFallback
      ? `${intent} — game titles only (add a RAWG key for cover art & scores)`
      : intent;
