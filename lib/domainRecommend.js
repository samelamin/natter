 ...
 */
async function generateGameCandidates(query, llmClient, platformLabels = []) {
  try {
    const platLine = platformLabels.length
      ? ` Only list games actually available on: ${platformLabels.join(', ')}.`
      : '';
      ...
      messages: [
        {
          role: 'system',
          content:
            'List 16 real, existing video games that match the user request.' +
            platLine +
    ...
    return games.slice(0, 16).map((g, i) => {
...
export async function domainRecommend({
  ...
  try {
    ...
    if (provider && typeof provider.search === 'function' && domain === 'game') {
      ...
      const synth = await generateGameCandidates(query, llmClient, platformLabels);
      let enriched = [];
      if (synth.length) {
        const lookups = await Promise.allSettled(
          synth.slice(0, 16).map(async (s) => {
