...
export async function search({ query, limit = 20, filters = {} } = {}) {
  ...
  const ck = `prov:game:search:${safe.toLowerCase()}|${limit}|${filters.genres || ''}`;
  const cached = await cacheGetJSON(ck);
  if (cached) return cached;
  // `search` ranks by relevance; exclude port/version duplicates.
  const body = `search "${safe}"; fields ${FIELDS}; where version_parent = null; limit ${limit};`;
