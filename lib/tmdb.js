...
/**
 * Resolve a TMDB id → IMDb id ("tt…"), or null. Stremio clients (e.g. Nuvio)
 * key meta/stream resolution on IMDb ids, so the Stremio addon needs this map.
 * Cached + retried like every other TMDB call.
 */
export async function imdbId({ tmdbId, kind }) {
  // Interpolated into the upstream URL — guard against path injection.
  if (!/^\d+$/.test(String(tmdbId))) return null;
  const type = kind === 'tv' ? 'tv' : 'movie';
  const url = `${BASE}/${type}/${tmdbId}/external_ids?api_key=${TMDB_KEY}`;
  try {
    const data = await fetchJson(url);
...
/**
 * Resolve an IMDb id ("tt…") → TMDB numeric id for the given kind, or null.
 * Inverse of imdbId(); needed by the Stremio `meta` resource, whose incoming id
 * is the IMDb id.
 */
export async function findByImdb({ imdb, type }) {
  if (!/^tt\d+$/.test(String(imdb))) return null;
  const url = `${BASE}/find/${imdb}?api_key=${TMDB_KEY}&external_source=imdb_id`;
  try {
    const data = await fetchJson(url);
...
/**
 * Every episode of a series across all seasons (specials / season 0 excluded),
 * as a flat, season/episode-sorted list — used to build a Stremio `meta` videos
 * array. Each entry: { season, episode, name, overview, air_date, still }.
 */
export async function getSeriesEpisodes({ tmdbId }) {
  if (!/^\d+$/.test(String(tmdbId))) return [];
  let details;
  try {
    details = await fetchJson(`${BASE}/tv/${tmdbId}?api_key=${TMDB_KEY}`);
  } catch {
    return [];
  }
  ...
  await Promise.all(
    seasons.map(async (s) => {
      try {
        const sd = await fetchJson(`${BASE}/tv/${tmdbId}/season/${s.season_number}?api_key=${TMDB_KEY}`);
        ...
        // Skip a season we couldn't fetch — better a partial list than none.
      }
    }),
  );
...
function movieResultToPick(m) {
  return {
    ...
    originCountry: Array.isArray(m.origin_country) ? m.origin_country : [],
...
function tvResultToPick(t) {
  return {
    ...
    originCountry: Array.isArray(t.origin_country) ? t.origin_country : [],
