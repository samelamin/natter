...
import { CATEGORY_CAP, shouldStopDeepening } from './paginate.js';
...
export function closeTitleMatches(results, requestedTitle) {
  const wanted = normalizeTitle(requestedTitle);
  if (!wanted) return results;
  const exact = results.filter((p) => normalizeTitle(p.title) === wanted);
  if (exact.length > 0) {
    return [...exact].sort((a, b) => (b._vote_count || 0) - (a._vote_count || 0)).slice(0, 1);
  }
  return results;
}
...
export function applyFilters(picks, kind, constraints) {
  ...
  if (constraints.plainComedy) {
    const offTone = ['Crime', 'Thriller', 'Horror', 'Sci-Fi', 'Sci-Fi & Fantasy', 'Action', 'Action & Adventure'];
    out = out.filter((p) => !offTone.some((g) => pickMatchesGenre(p, g)));
  }
...
export function extractConstraints(query) {
  ...
  const asksForNew =
    /\b(new|newer|newest|latest|recent|released|release|ott|streaming|current)\b/.test(q);
  ...
  const pastMonths = q.match(/past\s+(\d+)\s+months?/i) || q.match(/last\s+(\d+)\s+months?/i);
  if (pastMonths) {
    constraints.yearMin = CURRENT_YEAR;
  } else if (asksForNew && constraints.yearMin == null) {
    constraints.yearMin = CURRENT_YEAR - 1;
  }
  if (/\b(indian|india|bollywood|hindi|tamil|telugu|malayalam|kannada|marathi)\b/i.test(q)) {
    constraints.originCountry = 'IN';
  }
  ...
  if (!asksForNew && constraints.yearMax == null) {
    constraints.yearMax = CURRENT_YEAR - 1;
  }
  ...
  const genreMap = [
    ...
    { terms: ['horror', 'scary', 'spooky', 'creepy', 'رعب'], genre: 'Horror' },
    { terms: ['comedy', 'comedies', 'funny', 'hilarious', 'laugh', 'كوميدي'], genre: 'Comedy' },
    { terms: ['drama', 'dramas', 'emotional', 'sad', 'cry', 'tearjerker', 'tear-jerker', 'moving', 'دراما'], genre: 'Drama' },
    ...
    { terms: ['romance', 'romantic', 'date night', 'رومانسي'], genre: 'Romance' },
    { terms: ['family', 'kids', 'children', 'child-friendly', 'عائلي'], genre: 'Family' },
  ];
  ...
  // Mood phrasing that does not name a TMDB genre directly. These terms are
  // especially important for fallbackSearch(): when the model/API path fails,
  // an unconstrained discover call otherwise returns the same popular titles
  // for unrelated prompts.
  const moodMap = [
    { re: /\b(feel[-\s]?good|comfort(?:ing)?|cozy|cosy|uplifting|light[-\s]?hearted|wholesome)\b/, genres: ['Comedy'] },
    { re: /\b(mind[-\s]?bending|brain[-\s]?twist(?:ing)?|twisty|twist ending|puzzle|trippy)\b/, genres: ['Sci-Fi', 'Thriller'] },
    { re: /\b(tense|suspenseful|edge of (?:my|your|the) seat|gripping)\b/, genres: ['Thriller'] },
    { re: /\b(dark|bleak|gritty)\b/, genres: ['Drama'] },
    { re: /\b(adventure|quest|epic)\b/, genres: ['Adventure'] },
  ];
  for (const { re, genres } of moodMap) {
  ...
  const asksForDarkComedyContext = /\b(dark|bleak|gritty|crime|thriller|horror|scary|violent|sci[-\s]?fi|action)\b/i.test(q);
  if (constraints.requireGenres?.[0] === 'Comedy' && !asksForDarkComedyContext) {
    constraints.plainComedy = true;
  }
...
export function demoteGenresFor(query) {
  ...
  const demote = asksForAnimation ? [] : ['Animation'];
  const asksForComedy = /\b(comed(?:y|ies)|funny|hilarious|laugh|feel[-\s]?good|comfort(?:ing)?|co[sz]y|light[-\s]?hearted|wholesome)\b/i.test(q);
  const asksForDark = /\b(dark|bleak|gritty|crime|thriller|horror|scary|violent)\b/i.test(q);
  if (asksForComedy && !asksForDark) demote.push('Crime', 'Thriller', 'Horror');
  return demote;
}
...
function deterministicReason(pick, query, constraints) {
  const bits = [];
  const primaryGenre = constraints.requireGenres?.[0];
  if (primaryGenre && pickMatchesGenre(pick, primaryGenre)) bits.push(primaryGenre);
  if (constraints.originCountry === 'IN') bits.push('Indian');
  if (constraints.yearMin && pick.year && pick.year >= constraints.yearMin) bits.push('recent');
  if (pick.rating && pick.rating >= 7.5) bits.push('highly rated');
  const type = pick.kind === 'tv' ? 'series' : 'film';
  if (bits.length === 0) return `A ${type} that fits the request.`;
  return `A ${bits.join(', ')} ${type} for "${query}".`;
}

function hasExplicitCountryConstraint(constraints) {
  return !!constraints.originCountry;
}

function violatesCountry(pick, constraints) {
  if (!hasExplicitCountryConstraint(constraints)) return false;
  const countries = Array.isArray(pick.originCountry) ? pick.originCountry : [];
  // TMDB movie search/discover results often omit origin_country even when the
  // discover query was already country-filtered. Treat missing country as
  // unknown, not a violation; reject only a clear mismatch.
  return countries.length > 0 && !countries.includes(constraints.originCountry);
}

function matchesCompoundGenres(pick, constraints) {
  const genres = constraints.requireGenres || [];
  if (genres.length < 2) return true;
  const [, secondary] = genres;
  if (pickMatchesGenre(pick, secondary)) return true;

  // TMDB TV has no Thriller genre. For "sci-fi thriller", keep sci-fi shows
  // that carry mystery tension, and reject broad fantasy/action/comedy
  // titles that only match the sci-fi taxonomy bucket.
  if (secondary === 'Thriller' && pick.kind === 'tv') {
    const hasTension = pickMatchesGenre(pick, 'Mystery');
    const offTone = ['Comedy', 'Family', 'Crime', 'Action & Adventure'].some((g) => pickMatchesGenre(pick, g));
    return hasTension && !offTone;
  }

  return false;
}

export function validatePicksForQuery(picks, query, kind, constraints, { exact = false } = {}) {
  if (!Array.isArray(picks) || picks.length === 0) return [];
  let out = picks.filter((p) => p && p.id && p.title && p.poster);
  out = applyFilters(out, exact ? kind : 'all', constraints);
  out = out.filter((p) => !violatesCountry(p, constraints));
  out = out.filter((p) => matchesCompoundGenres(p, constraints));

  // For specific/comparison prompts, never let a later fallback reintroduce
  // broad genre padding. The grounding happens before this helper; this keeps
  // only the grounded, constraint-compatible items that survived.
  if (SPECIFIC_QUERY_RE.test(query || '') || exact) {
    return out;
  }
...
// ── Pool partial picks (pure helper) ─────────────────────────────────────────

/**
 * Derive a pool-ranked partial pick list from a raw candidates array.
 *
 * PURE: takes plain data (no closures, no network), returns a plain array.
 * Shape is identical to what the fast-path onPartial emits (rankAndBadge output).
 *
 * @param {Array} candidates - Array of pick objects (e.g. Array.from(metaCache.values())).
 *   Caller is responsible for pre-sorting by the desired pool order
 *   (e.g. byScore) before passing in — the helper preserves that order after
 *   filtering and delegates final sort/badge assignment to rankAndBadge.
 * @param {{ kind?: 'all'|'film'|'tv', limit?: number }} [opts]
 * @returns {Array} Ranked picks in rankAndBadge output shape.
 */
export function poolPartialPicks(candidates, { kind = 'all', limit = 10 } = {}) {
  if (!Array.isArray(candidates) || candidates.length === 0) return [];

  // Basic quality filter: must have a title and a poster
  let filtered = candidates.filter((p) => p && p.title && p.poster);

  // Optional kind filter
  if (kind === 'film') filtered = filtered.filter((p) => p.kind === 'film');
  else if (kind === 'tv') filtered = filtered.filter((p) => p.kind === 'tv');

  if (filtered.length === 0) return [];

  // rankAndBadge assigns match %, badge, and strips _vote_count —
  // producing the exact shape the fast-path partial emits.
  return rankAndBadge(filtered, limit);
}
  ...
  if (kind !== 'tv') {
    fetches.push(
      tmdbDiscover({
        ...
        originCountry: constraints.originCountry,
...
export const SPECIFIC_QUERY_RE = new RegExp(
  '\\b(?:starring|featuring|directed by)\\s+[A-Z][a-z]+' + // unambiguous credit markers
    ...
    '|\\b(?:set in|based on|about|similar(?:\\s+\\w+){0,3}\\s+to|in the style of)\\b' + // setting/theme/comparison
    '|(?<!\\bfeel\\s)(?<!\\bfeels\\s)(?<!\\bwould\\s)\\blike\\b' + // comparison "like X", not "feel like X"
    '|\\b(?:بطولة|ببطولة|اخراج|إخراج)\\b' + // Arabic credit markers
    '|(?:عادل|عاد|عادة)\\s+الإمام', // common Adel Imam voice/transcription variants
  'i',
);
...
export function searchKindFor(query, kindArg = 'all') {
  ...
  if (/\b(new|latest|released|release|ott|streaming)\b/i.test(query || '')) return wording;
...
export function isIncompleteQuery(query, constraints, prior) {
  if (prior) return false;
  const q = String(query || '').trim();
  if (!q || /^[\s.!?]+$/.test(q)) return true;
  const hasConstraint = !!(
    constraints.requireGenres?.length ||
    constraints.yearMin != null ||
    constraints.runtimeMaxMin != null ||
    constraints.originCountry
  );
  if (/عادة\s+الإمام/.test(q) && !/\b(?:بطولة|ببطولة|اخراج|إخراج)\b/.test(q)) return true;
  if (hasConstraint || SPECIFIC_QUERY_RE.test(q)) return false;
  if (/^(?:okay,\s*)?i(?:'|’)?m looking for (?:a |an )?(?:movie|film|show|series)\.?$/i.test(q)) return true;
  if (/^where\s+there(?:'|’)s\s+(?:a\s+)?man\s+with\s+the\.?$/i.test(q)) return true;
  return false;
}
...
export async function recommend({ query, kind: kindArg = 'all', services = [], onStep, onCandidates, onPartial, prior, excludeIds }) {
  const constraints = extractConstraints(query);
  if (isIncompleteQuery(query, constraints, prior)) {
    return {
      intent: query,
      steps: ['Reading your request'],
      picks: [],
      kind: kindFromQuery(query, kindArg),
      lang: languageFromQuery(query),
      providers: [],
    };
  }
  ...
  const isRecallQuery = /\b(remember|called|red pill|blue pill)\b/i.test(query);
  ...
  async function executeTool(name, args) {
    ...
    if (name === 'tmdb_search') {
      ...
      const requestedSearchKind = args.kind === 'tv' || args.kind === 'film' ? args.kind : 'all';
      const searches =
        requestedSearchKind === 'all'
          ? ['film', 'tv'].map((t) => tmdbSearch({ title: args.title, kind: t, language: contentLang, limit: 8 }))
          : [tmdbSearch({ title: args.title, kind: requestedSearchKind, language: contentLang, limit: 8 })];
      const raw = (await Promise.all(searches)).flat();
      const picks = closeTitleMatches(raw, args.title);
      cacheAll(picks);
      if (isRecallQuery && recallLeadIds.length === 0 && picks[0]?.id) {
        recallLeadIds.push(picks[0].id);
      }
      return JSON.stringify(picks.map(toCompact));
    }

    if (name === 'tmdb_discover') {
      if (SPECIFIC_QUERY_RE.test(query)) {
        return JSON.stringify({
          error: 'Generic discovery is disabled for specific/comparison requests. Use web_search plus tmdb_search, then finalize only grounded candidates.',
        });
      }
      const label = [
        args.genre,
  ...
  if (/\b(red pill|blue pill)\b/i.test(query)) {
    const matrix = closeTitleMatches(
      await tmdbSearch({ title: 'The Matrix', kind: 'film', language: contentLang, limit: 5 }),
      'The Matrix',
    );
    cacheAll(matrix);
    if (matrix[0]?.id) recallLeadIds.push(matrix[0].id);
  }
  ...
  let deterministicFastPath = false;
  ...
  try {
    if (plainQuery) {
      ...
      if (searchKind !== 'tv') {
        discoverFetches.push(
          tmdbDiscover({ kind: 'film', genre: primaryGenre, yearMin: constraints.yearMin, yearMax: constraints.yearMax, originCountry: constraints.originCountry, originalLanguage: contentLang, language: contentLang, providers: providerIds, limit: 30 }),
        );
      }
      if (searchKind !== 'film') {
        discoverFetches.push(
          tmdbDiscover({ kind: 'tv', genre: primaryGenre, yearMin: constraints.yearMin, yearMax: constraints.yearMax, withoutGenres: tvExclude2, originCountry: constraints.originCountry, originalLanguage: contentLang, language: contentLang, providers: providerIds, limit: 30 }),
        );
      }
      ...
      if (metaCache.size === 0) {
        ...
        if (onPartial && poolPicks.length >= 4 && providerIds.length === 0) {
          const partial = validatePicksForQuery(poolPicks, query, 'all', constraints);
          onPartial({
            kind: requestedKind,
            picks: rankAndBadge(partial, 40, boostGenres, demoteGenres),
          });
        }

        deterministicFastPath = true;
        const vetted = validatePicksForQuery(poolPicks, query, 'all', constraints);
        finalizeResult = {
          intent: query,
          steps: ['Scanning the catalogue', 'Picking the best fits'],
          picks: vetted
            .slice(0, 24)
            .map((p) => ({ id: p.id, reason: deterministicReason(p, query, constraints) })),
        };
      }
    }
    ...
    if (!plainQuery || metaCache.size === 0) {
      // ── Full ReAct loop ───────────────────────────────────────────────────
      finalizeResult = null;
      // Track whether we've already emitted a ReAct-path partial so we fire
      // at most once (the finalize-step emit at line ~1202 is the second event).
      let reactPartialEmitted = false;
      ...
      // Emit a pool-ranked partial NOW — before the synthesis LLM call — so
      // the client can show browseable results while the model reasons.
      // Guarded: onPartial present, no prior partial this run, no provider
      // filter (same guard as the fast-path and finalize-step partials), and
      // poolPartialPicks must yield at least 4 picks to be worth showing.
      // The fast-path partial (plainQuery branch) is unreachable here because
      // this block only executes when !plainQuery || metaCache.size === 0, and
      // the fast-path partial is inside the plainQuery && metaCache.size > 0 branch.
      if (forceFinalize && onPartial && !reactPartialEmitted && providerIds.length === 0) {
        const poolCandidates = Array.from(metaCache.values())
          .sort(byScore(boostGenres, demoteGenres));
        const partialPicks = poolPartialPicks(poolCandidates, { kind: 'all', limit: 40 });
        if (partialPicks.length >= 4) {
          try {
            onPartial({ kind: requestedKind, intent: '', picks: partialPicks });
          } catch (err) {
            console.warn('[agent] onPartial threw (react pool partial):', err.message);
          }
          reactPartialEmitted = true;
        }
      }
      ...
      // If no finalize after the loop, synthesize from what's cached
        ...
        if (SPECIFIC_QUERY_RE.test(query) && /\b(new|newer|newest|latest|recent|released|release|ott|streaming|current)\b/i.test(query)) {
          return {
            intent: query,
            steps: ['Reading your request', 'Searching for exact matches'],
            picks: [],
            kind: requestedKind,
            lang: contentLang,
    ...
    if (recallLeadIds.length > 0) {
      const lead = metaCache.get(recallLeadIds[0]);
      if (lead && resolvedPicks.some((p) => p.id === lead.id)) {
        resolvedPicks = [
          { ...lead, reason: 'This is the most likely title you were remembering.' },
          ...resolvedPicks.filter((p) => p.id !== lead.id),
        ];
      } else if (lead) {
        resolvedPicks = [
          { ...lead, reason: 'This is the most likely title you were remembering.' },
    ...
    if (onPartial && resolvedPicks.length >= 4 && providerIds.length === 0) {
      onPartial({
        ...
        picks: rankAndBadge(validatePicksForQuery(resolvedPicks, query, 'all', constraints), 40, boostGenres, demoteGenres),
      });
    }
    ...
    let filtered = validatePicksForQuery(resolvedPicks, query, isExactTitles ? requestedKind : 'all', constraints, { exact: isExactTitles });
    ...
    // Deepening fill: page through discover until each category reaches its
    // target depth OR relevance thins out (honest depth — a cap, not a quota).
    // In-scope categories (per searchKind) aim for CATEGORY_CAP so the user can
    // page through ~45; the other tab keeps a baseline so it isn't a dead end.
    // Each round streams the growing pool via onPartial, so the client's later
    // pages fill in the background while the user browses page 1. Skipped for
    // exactTitles (actor/franchise) where genre padding is wrong.
    if (!deterministicFastPath && !isExactTitles && ((constraints.requireGenres && constraints.requireGenres.length > 0) || contentLang)) {
      const PER_TYPE_BASELINE = 18;
      const DEEP_BUDGET_MS = 6000;
      ...
      // animation and buries live-action. (The ranking demote is a backstop.)
      const tvExclude = demoteGenres.map((g) => TV_GENRE_IDS[g]).filter(Boolean);
      const score = byScore(boostGenres, demoteGenres);
      // In-scope categories deepen to the cap; the other tab stays at a baseline.
      const targetFor = (t) => (searchKind === 'all' || searchKind === t ? CATEGORY_CAP : PER_TYPE_BASELINE);
      const countOf = (t) => filtered.filter((p) => p.kind === t).length;
      const done = { film: countOf('film') >= targetFor('film'), tv: countOf('tv') >= targetFor('tv') };

      for (let page = 1; page <= 6; page++) {
        if (done.film && done.tv) break;
        if (Date.now() - deepStartedAt > DEEP_BUDGET_MS) break;
        const fetchTypes = ['film', 'tv'].filter((t) => !done[t]);
        if (page === 1 && onStep) onStep(`Finding more${primaryGenre ? ` ${primaryGenre}` : ''} picks`);
        ...
        for (const { t, more } of rounds) {
          cacheAll(more);
          const before = countOf(t);
          filtered = dedupeByTitle([...filtered, ...validatePicksForQuery(more, query, 'all', constraints)]);
          const added = countOf(t) - before;
          // Page 1 overlaps heavily with the model's already-surfaced popular
          // titles, so a low "added" there is duplication, not a dry genre —
          // only let the thin-supply stop bite from page 2 onward (minNew: 0).
          if (shouldStopDeepening({ total: countOf(t), added, target: targetFor(t), minNew: page === 1 ? 0 : 3 })) {
            done[t] = true;
          }
        }

        // Progressive paint: stream the growing, balanced pool so later pages are
        // ready as the user browses page 1. Skipped on filtered runs (must not
        // flash unfiltered content), as with the other partial emits.
        if (onPartial && providerIds.length === 0) {
          const f = filtered.filter((p) => p.kind === 'film').sort(score).slice(0, CATEGORY_CAP);
          const tvp = filtered.filter((p) => p.kind === 'tv').sort(score).slice(0, CATEGORY_CAP);
          onPartial({
            kind: requestedKind,
            intent: finalizeResult.intent,
            phase: 'deepening',
            picks: rankAndBadge([...f, ...tvp], CATEGORY_CAP * 2, boostGenres, demoteGenres),
          });
        }
      }
      console.log(
        `[agent] deepening fill: film ${countOf('film')}, tv ${countOf('tv')} (targets ${targetFor('film')}/${targetFor('tv')})`,
      );
    }

    // Relax if too few results — but NOT for exactTitles (return honest empty instead of padding)
    const hasHardUserConstraint =
      deterministicFastPath ||
      constraints.originCountry ||
      constraints.yearMin != null ||
      constraints.yearMax != null ||
      constraints.runtimeMaxMin != null;
    if (!hasHardUserConstraint && !isExactTitles && filtered.length < 3 && Object.keys(constraints).length > 0) {
    ...
    // ~CATEGORY_CAP of each type, then rankAndBadge interleaves by rating. The
    // client paginates each category 9 at a time over this pool.
    const score = byScore(boostGenres, demoteGenres);
    const films = filtered.filter((p) => p.kind === 'film').sort(score).slice(0, CATEGORY_CAP);
    const tv = filtered.filter((p) => p.kind === 'tv').sort(score).slice(0, CATEGORY_CAP);
    let rankedPicks = rankAndBadge([...films, ...tv], CATEGORY_CAP * 2, boostGenres, demoteGenres);
    if (/\b(red pill|blue pill)\b/i.test(query)) {
      const matrixIndex = rankedPicks.findIndex((p) => p.kind === 'film' && p.title === 'The Matrix');
      if (matrixIndex > 0) {
        const [matrix] = rankedPicks.splice(matrixIndex, 1);
        rankedPicks.unshift({ ...matrix, badge: { label: 'Top pick', variant: 'gold' }, match: 99 });
      }
    }
