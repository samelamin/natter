...
export async function recommend({ query, kind: kindArg = 'all', onStep }) {
  ...
  const messages = [
    { role: 'system', content: SYSTEM_PROMPT },
    {
      role: 'user',
      content:
        `Find recommendations for: "${query}"\nKind: ${kindHint}` +
        (queryLang
  ...
  try {
    ...
    if (!isExactTitles && ((constraints.requireGenres && constraints.requireGenres.length > 0) || contentLang)) {
      ...
      for (const t of types) {
        ...
        if (onStep) onStep(`Finding ${t === 'tv' ? 'shows' : 'films'}${primaryGenre ? ` — ${primaryGenre}` : ''}`);
    ...
    let rankedPicks = rankAndBadge(finalPicks, kind === 'all' ? 40 : 24, boostGenres, demoteGenres);
