...
export function extractConstraints(query) {
  ...
  const genreMap = [
    ...
    { terms: ['sci-fi', 'scifi', 'sci fi', 'science fiction'], genre: 'Sci-Fi' },
...
export function demoteGenresFor(query) {
  return /\b(animat\w*|anime|cartoon)\b/i.test(query || '') ? [] : ['Animation'];
}
...
async function fallbackSearch(query, kind) {
  ...
  return {
    ...
    picks: rankAndBadge(deduped, 24, boostGenres, demoteGenres),
  };
}
  ...
  try {
    for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
      ...
      for (const tc of cappedCalls) {
        messages.push({
          ...
          content: JSON.stringify({
            error: `tmdb_search limit (${MAX_SEARCH_PER_ROUND} per round) reached — use the results you already have or call finalize.`,
          }),
        });
      }
      ...
      if (finalizeCall) {
        let args = null;
        try {
          args = JSON.parse(finalizeCall.function.arguments);
        } catch (err) {
          // Malformed finalize must not abort the run — tell the model and
          // let it retry on the next round.
          console.warn('[agent] malformed finalize args:', err.message);
          messages.push({
            role: 'tool',
            tool_call_id: finalizeCall.id,
            content: JSON.stringify({ error: 'finalize arguments were not valid JSON — call finalize again.' }),
          });
          continue;
        }
    ...
    for (const { id, reason } of rawPicks || []) {
      ...
      if (!meta && !/^tmdb:\d+$/.test(String(id))) {
        console.log(`[agent] id ${id} not in cache, attempting title recovery`);
        const recovered = await tmdbSearch({ title: String(id), kind: 'film', limit: 3 });
    ...
    const rankedPicks = rankAndBadge(finalPicks, kind === 'all' ? 40 : 24, boostGenres, demoteGenres);
